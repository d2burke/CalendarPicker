jQuery.fn.calendarPicker = function(options) {
  ...
  if(calendar.options.vertical){
	  //ADD CLASSES TO SHIFT DIALS TO THE VERTICAL POSITION
	  divYears.addClass('vertical');
	  divMonths.addClass('vertical');
	  divDays.addClass('vertical');
  }
  ...
  calendar.changeDate = function(date) {
    ...
    var fillDays = function(date) {
      ...
      var w = (options.vertical) ? parseInt((options.width / 3) - 6)+"px" : parseInt((theDiv.width()-4-(nc)*4)/nc)+"px";
      var h = (parseInt((theDiv.width()-4-(nc)*4)/nc) - 15)+"px";
	  ...
	  //SET STANDARD ELEMENT HEIGHT TO DAY CELL HEIGHT BECAUSE IT IS THE TALLEST
	  options.elementHeight = span.height();
    }
    ...
    var fillYears = function(date) {
      ...
      var w = (options.vertical) ? parseInt((options.width / 3) - 6)+"px" : parseInt((theDiv.width()-4-(nc)*4)/nc)+"px";
      var h = options.elementHeight - 10;
      for (var i = year - options.years; i <= year + options.years; i++) {
        var d = new Date(date);
        d.setFullYear(i);
        var span = $("<span>").addClass("calElement").attr("millis", d.getTime()).html(i).css("width",w).css("height",h);
    ...
    var fillMonths = function(date) {
      ...
      var w = (options.vertical) ? parseInt((options.width / 3) - 6)+"px" : parseInt((theDiv.width()-4-(nc)*4)/nc)+"px";
      var h = options.elementHeight - 15;
      for (var i = -options.months; i <= options.months; i++) {
        ...
        var span = $("<span>").addClass("calElement").attr("millis", d.getTime()).html(options.monthNames[d.getMonth()]).css("width",w).css("height",h);
